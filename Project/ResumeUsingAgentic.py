...
def shortlist_window():
    ...
    if HR_query:
        s.markdown(f"**HR Query:** {HR_query}")

        # Create agent with LLM and resume retrieval tool
        agent = create_agent(
            model=llm,
            tools=[get_top_resumes],
            system_prompt="""
            ...
            IMPORTANT RULES:
            1. Only analyze resumes provided by the tool.
            2. Do NOT hallucinate or add information not present in resumes.
            3. If the query is NOT recruitment-related, respond EXACTLY:
               "Please ask a recruitment or HR-like question."
            4. Select only top 1-2 most suitable candidates.
            5. Output structured JSON: Selected Applicant(s) info, Why Selected, Why Not Selected
            """
        )

        # Prompt for agent
        agent_prompt = f"""
        Analyze the following HR query and fetch top resumes using the tool:
        HR Query: {HR_query}
        ...
        # Get agent output
        explanation_output = result["messages"][-1].content.strip()

        s.markdown("### Shortlisting Results")
